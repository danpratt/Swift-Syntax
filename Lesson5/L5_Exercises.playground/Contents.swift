...
func returnLastLetterOfString(string: String) -> Character {
    var string = string
    return string.remove(at: string.index(before: string.endIndex))
}
//:Test out your discovery below by returning the last letter of the String, "bologna".
var word = "bologna"

returnLastLetterOfString(string: word)
...
func combineLastCharacters(strings: [String]) -> String {
    var stringToReturn = ""
    for word in strings {
        let letter = returnLastLetterOfString(string: word)
        stringToReturn += String(letter)
    }
...
func isOnlyDigits(price: String) -> Bool {
...
func washYourMouthOutWithSoap(wordsToClean: [String]) -> [String] {
...
print(washYourMouthOutWithSoap(wordsToClean: dirtyWordsArray))
//: __Problem 5__
//:
...
let moviesByCN = myMovies.filterByDirector(movies: movies, director: "Christopher Nolan")
print(moviesByCN)
