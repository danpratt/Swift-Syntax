...
let yourSentence = "The \(nounArray[0]) had a \(nounArray[4]) and ate my \(nounArray[1])"
let yourSillySentence = "When the \(nounArray[index1]) saw the \(nounArray[index2]), he exclaimed, 'It's you!'"
//: ### Exercise 2
...
let charactersInString = howManyCharacters.characters.count
//: ### Exercise 4
...
for character in gString.lowercased().characters {
    if character == "g" {
        count += 1
    }
}
...
print("Josie has saved $\(dollars).\(cents)")
//: # Let or Var?
import UIKit
import Foundation
//: ### Exercise 9
