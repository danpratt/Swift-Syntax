...
let reversedNames = surnames.sorted(by: {$0 > $1 })
print(reversedNames)
...
let reversedAverages = battingAverages.sorted(by: {$0 > $1})
...
let divisibleBy12 = numbersAsStrings.filter( { Int($0)! % 12 == 0 } )
...
let tinyParticles = particleSizesInMicrons.filter({ $0 < 20 })
print(tinyParticles)
...
let pricesInDollars = pricesInRupees.map({ $0 / 64 })
